...
argument_list|()
decl_stmt|;
comment|/**      * {@link State} persisted to data directory of features.core bundle.      */
specifier|private
specifier|final
name|State
name|state
...
name|print
argument_list|(
literal|"Skipping blacklisted feature: "
operator|+
name|f
argument_list|,
name|options
...
expr_stmt|;
block|}
comment|/**      * Actual deployment needs to be done in a separate thread.      * The reason is that if the console is refreshed, the current thread which is running      * the command may be interrupted while waiting for the refresh to be done, leading      * to bundles not being started after the refresh.      *      * @param requirements the provided requirements to match.      * @param stateChanges the current features state.      * @param state the current provisioning state.      * @param options the provisioning options.      * @param wait wait for provisioning to complete      * @throws Exception in case of provisioning failure.      */
specifier|private
name|void
name|doProvisionInThread
parameter_list|(
