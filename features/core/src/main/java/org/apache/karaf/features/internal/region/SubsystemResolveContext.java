...
name|nbMandatory
decl_stmt|;
comment|// Iterate while we find more mandatory resources
do|do
block|{
name|nbMandatory
operator|=
...
control|)
block|{
comment|// Check mandatory requirements of mandatory resources
for|for
control|(
name|Requirement
name|req
...
block|{
comment|// In case there are multiple providers
comment|// check if there is a single provider which has
comment|// a mandatory identity requirement on a mandatory
comment|// resource, in which case we also assume this one
...
argument_list|)
expr_stmt|;
comment|// If the repository if of type XmlRepository, then it is our own internal format
if|if
condition|(
name|repository
operator|instanceof
