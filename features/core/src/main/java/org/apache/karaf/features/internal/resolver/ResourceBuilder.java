...
argument_list|()
decl_stmt|;
comment|// Since all R3 exports imply an import, add a corresponding
comment|// requirement for each existing export capability. Do not
comment|// duplicate imports.
name|Map
argument_list|<
name|String
argument_list|,
...
block|}
block|}
comment|// Add import requirement for each export capability.
for|for
control|(
name|BundleCapability
name|export
...
condition|)
block|{
comment|// Convert Version to VersionRange.
name|Object
name|version
init|=
name|export
...
name|BundleException
block|{
comment|// Add a "uses" directive onto each export of R3 bundles
comment|// that references every other import (which will include
comment|// exports, since export implies import); this is
comment|// necessary since R3 bundles assumed a single class space,
comment|// but R4 allows for multiple class spaces.
name|String
name|usesValue
init|=
literal|""
