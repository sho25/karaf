...
name|Deployer
block|{
comment|/**      * Interface through which {@link Deployer} interacts with OSGi framework.      */
specifier|public
interface|interface
name|DeployCallback
extends|extends
...
specifier|static
class|class
name|CircularPrerequisiteException
extends|extends
name|Exception
block|{
specifier|private
...
block|}
block|}
comment|/**      *<p>Representation of the state of system from the point of view of<em>installed bundles</em>      * and<em>available features</em></p>      */
specifier|public
specifier|static
class|class
name|DeploymentState
block|{
comment|// part of the deployment state related to features service
comment|/** Current {@link State} of features service */
specifier|public
name|State
name|state
decl_stmt|;
comment|// part of the deployment state related to low level OSGi framework (bundles, no regions)
...
name|bundles
decl_stmt|;
comment|// part of the deployment state related to all available features
comment|/** feature-name -&gt; list of features for different versions for all available features (not only installed) */
specifier|private
name|Map
argument_list|<
name|String
...
name|features
decl_stmt|;
comment|/** feature-id -&gt; feature (not only installed) */
specifier|private
name|Map
argument_list|<
name|String
...
name|featuresById
decl_stmt|;
comment|// part of the deployment state related to regions
comment|/** region-name -&gt; ids for bundles installed in region (see {@link State#managedBundles}) */
specifier|public
name|Map
argument_list|<
name|String
...
decl_stmt|;
comment|/**          * Returns all features indexed by their name. For each name we have collection of {@link Feature features}          * for different versions.          * @return          */
specifier|public
name|Map
argument_list|<
name|String
...
return|;
block|}
comment|/**          * Returns all features indexed by their id.          * @return          */
specifier|public
name|Map
argument_list|<
name|String
...
return|;
block|}
comment|/**          * Sets a list of features and stores it as map of features where the key is<code>name</code> and value is a          * list of features with different versions.          * @param featuresList          */
specifier|public
name|void
...
block|}
block|}
comment|/**      *<p>A request to change current {@link DeploymentState} of system</p>      *<p>{@link #requirements} specify target set of system requirements. If new features are installed,      * requirements should include currently installed features and new ones. If features are being uninstalled,      * requirements should include currently installed features minus the ones that are removed.</p>      */
specifier|public
specifier|static
class|class
name|DeploymentRequest
block|{
comment|/** A bnd macro that changes feature version into a version range. */
specifier|public
name|String
name|featureResolutionRange
decl_stmt|;
comment|/** Indication of how to handle requirements from<code>osgi.service</code> namespace */
specifier|public
name|FeaturesService
operator|.
...
decl_stmt|;
comment|/** A bnd macro to find update'able version range for bundle versions (e.g., to determine whether to install or update a bundle */
specifier|public
name|String
name|bundleUpdateRange
decl_stmt|;
comment|/** Indication of when to update bundles (or leave them as they are currently installed) */
specifier|public
name|FeaturesService
operator|.
...
name|updateSnaphots
decl_stmt|;
comment|/**          * Additional {@link Repository} that'll be used to resolve unresolved, non-optional requirements if          * they're not resolved against current          */
specifier|public
name|Repository
name|globalRepository
decl_stmt|;
comment|/** Target/desired set of requirements per region */
specifier|public
name|Map
argument_list|<
name|String
...
name|requirements
decl_stmt|;
comment|/** Target/desired set of features state per region */
specifier|public
name|Map
argument_list|<
name|String
...
name|options
decl_stmt|;
comment|/** File to store result of deployment */
specifier|public
name|String
name|outputFile
decl_stmt|;
comment|/**          * Prepare standard, empty DeploymentRequest, where feature versions are taken literally (no ranges)          * and bundle updates use<em>natural</em> range to determine between install and update (update on micro          * digit in version, e.g.,<code>2.1.0</code> -&gt;<code>2.1.2</code>, but not<code>2.1.2</code> -&gt;          *<code>2.2.0</code>).          * @return          */
specifier|public
specifier|static
name|DeploymentRequest
...
block|}
block|}
comment|/**      * Deployment information for all regions      */
specifier|static
class|class
name|Deployment
block|{
...
decl_stmt|;
block|}
comment|/**      * Deployment information for single region      */
specifier|static
class|class
name|RegionDeployment
block|{
comment|/** new {@link Resource resources} to install */
name|List
argument_list|<
name|Resource
argument_list|>
...
argument_list|()
decl_stmt|;
comment|/** existing {@link Bundle bundles} to remove */
name|List
argument_list|<
name|Bundle
argument_list|>
...
argument_list|()
decl_stmt|;
comment|/** existing {@link Bundle bundles} to update using new {@link Resource resources} */
name|Map
argument_list|<
name|Bundle
argument_list|,
...
expr_stmt|;
block|}
comment|/**      * Performs full deployment - with prerequisites      *      * @param dstate  deployment state      * @param request deployment request      * @throws Exception in case of deployment failure.      */
specifier|public
name|void
name|deployFully
parameter_list|(
...
comment|// current managed bundles per region, as known by o.a.k.features.internal.service.FeaturesServiceImpl.state
name|Map
argument_list|<
name|String
argument_list|,
...
argument_list|)
decl_stmt|;
comment|// current not managed (by FeaturesService state) bundles per region, as known by o.a.k.features.internal.service.BundleInstallSupportImpl.digraph
comment|// "unmanaged" means "not installed via features service"
name|Map
argument_list|<
name|String
argument_list|,
...
argument_list|)
decl_stmt|;
comment|// Use Subsystem and Felix resolver
name|SubsystemResolver
name|resolver
init|=
operator|new
...
argument_list|)
decl_stmt|;
comment|// preparation - creating OSGi resources with reqs and caps for regions and features
name|resolver
operator|.
name|prepare
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// if some features have prerequisites, we have to deploy them first - this method may throw Exception
comment|// to start another cycle of deployment
...
argument_list|)
expr_stmt|;
comment|// when there are no more prerequisites, we can resolve Subsystems and Features using Felix resolver
comment|// Subsystem resolver will have then full information about new bundles and bundle updates or removals
comment|// per region
name|resolver
operator|.
name|resolve
...
argument_list|)
decl_stmt|;
comment|// changes to current state - added and removed features
name|Map
argument_list|<
name|String
argument_list|,
...
block|}
block|}
comment|// Compute information for each bundle (region -> location -> BundleInfo)
name|Map
argument_list|<
name|String
argument_list|,
...
block|}
block|}
comment|// Put default Started state for other bundles if start attribute is true
for|for
control|(
name|Resource
name|resource
...
name|print
argument_list|(
literal|" Configurations to delete:"
argument_list|,
name|verbose
argument_list|)
expr_stmt|;
...
name|print
argument_list|(
literal|" Configuration Files to delete:"
argument_list|,
name|verbose
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|// Set start levels after install to avoid starting before all bundles are installed
for|for
control|(
name|Bundle
name|bundle
...
block|}
block|}
comment|// If uninstall and delete configurations, actually delete configurations and configuration files
...
decl_stmt|;
comment|// **********************************************************************
comment|// KARAF-6239: workaround to avoid several entries for the same resource
name|Map
argument_list|<
name|String
argument_list|,
...
name|debug
argument_list|(
literal|"Error calculating checksum for bundle: {}"
argument_list|,
name|bundle
argument_list|,
name|t
