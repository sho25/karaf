...
argument_list|)
expr_stmt|;
comment|/*                              * Do not use symbolic links for now, and do not put the jar                              * in the system folder                              *                             String mvnPath = Parser.pathFromMaven(provider.getUrl());                             Path sysOutput = systemDirectory.resolve(mvnPath);                             Files.createDirectories(sysOutput.getParent());                             Files.copy(input, sysOutput, StandardCopyOption.REPLACE_EXISTING);                             Path libOutput = homeDirectory.resolve(path).resolve(name);                             if (Files.notExists(libOutput, LinkOption.NOFOLLOW_LINKS)) {                                 try {                                     Files.createSymbolicLink(libOutput, libOutput.getParent().relativize(sysOutput));                                 } catch (FileSystemException e) {                                     Files.copy(input, libOutput, StandardCopyOption.REPLACE_EXISTING);                                 }                             }                             */
block|}
else|else
block|{
name|Path
