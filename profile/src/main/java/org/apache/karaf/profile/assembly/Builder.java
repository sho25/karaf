...
argument_list|)
expr_stmt|;
comment|// TODO: reformat to multiline values
name|featuresProperties
operator|.
name|save
argument_list|()
...
argument_list|)
expr_stmt|;
comment|//        location = DownloadManagerHelper.removeInlinedMavenRepositoryUrl(location);
if|if
condition|(
name|location
operator|.
...
comment|/*             Artifact artifact = dependencyHelper.mvnToArtifact(location);             if (artifact.isSnapshot()) {                 File metadataTarget = new File(targetFile.getParentFile(), "maven-metadata-local.xml");                 try {                     MavenUtil.generateMavenMetadata(artifact, metadataTarget);                 } catch (Exception e) {                     getLog().warn("Could not create maven-metadata-local.xml", e);                     getLog().warn("It means that this SNAPSHOT could be overwritten by an older one present on remote repositories");                 }             }             */
block|}
else|else
block|{
name|LOGGER
