...
expr_stmt|;
block|}
comment|//        if (!blacklistedFeatures.isEmpty() || !blacklistedBundles.isEmpty()) {
comment|//            List<String> lines = new ArrayList<>();
comment|//            lines.add("#");
comment|//            lines.add("# Generated by the karaf assembly builder");
comment|//            lines.add("#");
comment|//            if (!blacklistedFeatures.isEmpty()) {
comment|//                lines.add("");
comment|//                lines.add("# Features");
comment|//                lines.addAll(blacklistedFeatures);
comment|//            }
comment|//            if (!blacklistedBundles.isEmpty()) {
comment|//                lines.add("");
comment|//                lines.add("# Bundles");
comment|//                lines.addAll(blacklistedBundles);
comment|//            }
comment|//            LOGGER.info("Generating {}", homeDirectory.relativize(blacklist));
comment|//            Files.write(blacklist, lines, StandardCharsets.UTF_8, StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING);
comment|//        }
comment|// TODO: download overrides, implement fuller override clauses (original->replacement)
if|if
condition|(
name|processor
operator|.
...
argument_list|)
expr_stmt|;
comment|// TODO consult blacklist policy
comment|//                                if (blacklistPolicy == BlacklistPolicy.Discard) {
comment|//                                }
name|loaded
operator|.
name|put
