...
block|}
block|}
comment|/**      * TODO: remove this hack when FELIX-1628 is properly implemented      * Hack for FELIX-1626.      * FileInstall has some delay before installing the configurations which can cause some problems      *      * @param b      */
specifier|private
name|void
name|checkCmProperties
parameter_list|(
...
try|try
block|{
comment|// Try to load fileinstall internal classes.
comment|// Those are not exported, so this mean if the load succeeds, we have the fileinstall bundle
specifier|final
name|Class
name|ciClass
init|=
...
name|loadClass
argument_list|(
literal|"org.apache.felix.fileinstall.internal.ConfigInstaller"
argument_list|)
decl_stmt|;
specifier|final
name|Class
...
name|loadClass
argument_list|(
literal|"org.apache.felix.fileinstall.internal.FileInstall"
argument_list|)
decl_stmt|;
comment|// If we have been able to load the classes, start a thread that will wait until
comment|// fileinstall is correctly configured using config admin and push all configs.
operator|new
name|Thread
argument_list|()
block|{
...
literal|true
expr_stmt|;
comment|//                                System.err.println("Found ready FileInstall");
for|for
control|(
name|File
name|f
...
condition|)
block|{
comment|//                                        System.err.println("Processing: " + f.getName());
name|mth
operator|.
name|invoke
argument_list|(
