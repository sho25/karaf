...
condition|)
block|{
comment|// try table name in lower case
name|rs
operator|=
name|metadata
operator|.
...
expr_stmt|;
block|}
comment|/*             if (schemaExists == false) {                 // try table name in upper case                 rs = getConnection().getMetaData().getTables(null, null, tableName.toUpperCase(), new String[] {"TABLE"});                 schemaExists = rs.next();             }             */
block|}
catch|catch
parameter_list|(
name|Exception
