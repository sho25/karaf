...
argument_list|)
decl_stmt|;
comment|//                        bits = location.toString().split("[:/]");
comment|//                        if (bits.length< 4) {
comment|//                            getLog().warn("bad bundle: " + location);
comment|//                        } else {
comment|//                        Artifact bundleArtifact = factory.createArtifact(bits[1], bits[2], bits[3], null, bits.length == 4? "jar": bits[4]);
comment|//                        String bundlePath = location.startsWith("mvn:")? location.substring("mvn:".length()).replaceAll("/", ":"): location;
comment|//layout.pathOf(bundleArtifact);
if|if
condition|(
name|startupProperties
operator|.
