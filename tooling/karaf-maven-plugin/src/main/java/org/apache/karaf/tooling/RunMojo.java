...
literal|"false"
decl_stmt|;
comment|/**      * Maximum duration startup can take in milliseconds, negative or zero values mean no maximum.      */
annotation|@
name|Parameter
argument_list|(
name|defaultValue
...
annotation|@
name|Override
comment|// avoids to use that silently and fail later on in the waiting loop
specifier|protected
name|Class
argument_list|<
name|?
...
name|ClassNotFoundException
argument_list|(
literal|"avoid to use the classrealm loader which will prevent felix to match its reference"
argument_list|)
throw|;
block|}
return|return
...
name|ACTIVE
operator|&&
name|checkDurationTimeout
argument_list|(
name|start
argument_list|)
condition|)
block|{
name|waitForValidState
argument_list|()
expr_stmt|;
block|}
if|if
...
throw|;
block|}
comment|// first find the feature bundle to load the bootfinished class properly,
comment|// if we use bundleContext0.getBundle() we end up on ClassRealm which will not match in ServiceRegImpl
name|BundleContext
name|featureBundleCtx
init|=
literal|null
decl_stmt|;
name|Object
...
name|equals
argument_list|(
literal|"org.apache.karaf.deployer.features"
argument_list|)
argument_list|)
operator|.
name|findFirst
...
block|}
block|}
comment|// todo: maybe add it as a mojo parameter to reduce it for light distro?
specifier|private
name|void
name|waitForValidState
parameter_list|()
...
name|IllegalStateException
argument_list|(
literal|"Server didn't start in "
operator|+
name|TimeUnit
operator|.
name|NANOSECONDS
