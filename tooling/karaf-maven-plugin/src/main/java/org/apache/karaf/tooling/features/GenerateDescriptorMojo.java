...
decl_stmt|;
comment|/**      * Flag indicating whether bundles should use the version range declared in the POM. If<code>false</code>,      * the actual version of the resolved artifacts will be used.      */
annotation|@
name|Parameter
argument_list|(
name|defaultValue
...
specifier|private
name|boolean
name|useVersionRange
decl_stmt|;
comment|/**      * Flag indicating whether the plugin should determine whether transitive dependencies are declared with      * a version range. If this flag is set to<code>true</code> and a transitive dependency has been found      * which had been declared with a version range, that version range will be used to build the appropriate      * bundle element instead of the newest version. This flag has only an effect when {@link #useVersionRange}      * is<code>true</code>      */
annotation|@
name|Parameter
argument_list|(
name|defaultValue
...
specifier|private
name|boolean
name|includeTransitiveVersionRanges
decl_stmt|;
annotation|@
name|Parameter
specifier|private
...
decl_stmt|;
comment|/**      * Flag indicating whether the plugin should simplify bundle dependencies. If the flag is set to {@code true}      * and a bundle dependency is determined to be included in a feature dependency, the bundle dependency is      * dropped.      */
annotation|@
name|Parameter
argument_list|(
name|defaultValue
...
name|log
decl_stmt|;
comment|// If useVersionRange is true, this map will be used to cache
comment|// resolved MavenProjects
specifier|private
specifier|final
name|Map
argument_list|<
...
name|info
argument_list|(
literal|"Generation not enabled"
argument_list|)
expr_stmt|;
name|getLog
argument_list|()
...
name|info
argument_list|(
literal|"Attaching artifact"
argument_list|)
expr_stmt|;
comment|//projectHelper.attachArtifact(project, attachmentArtifactType, attachmentArtifactClassifier, outputFile);
name|Artifact
name|artifact
init|=
name|factory
...
name|info
argument_list|(
literal|"Attaching features XML"
argument_list|)
expr_stmt|;
...
name|project
operator|.
name|getPluginArtifactRepositories
argument_list|()
argument_list|)
expr_stmt|;
name|request
...
literal|false
decl_stmt|;
comment|// First pass to look for features
comment|// Track other features we depend on and their repositories (we track repositories instead of building them from
comment|// the feature's Maven artifact to allow for multi-feature repositories)
comment|// TODO Initialise the repositories from the existing feature file if any
name|Map
argument_list|<
name|Dependency
argument_list|,
...
expr_stmt|;
block|}
comment|// Do not retain cache beyond this point
name|cache
operator|=
literal|null
expr_stmt|;
comment|// Second pass to look for bundles
if|if
condition|(
name|addBundlesToPrimaryFeature
condition|)
...
condition|)
block|{
comment|// The bundle matches a configfile, ignore it
continue|continue
name|localDependency
continue|;
block|}
...
argument_list|)
expr_stmt|;
comment|// Check the features this feature depends on don't already contain the dependency
comment|// TODO Perhaps only for transitive dependencies?
name|boolean
name|includedTransitively
init|=
name|simplifyBundleDependencies
operator|&&
name|isBundleIncludedTransitively
argument_list|(
name|feature
argument_list|,
name|otherFeatures
...
expr_stmt|;
block|}
comment|// Add any missing repositories for the included features
for|for
control|(
name|Feature
...
argument_list|)
expr_stmt|;
comment|// Determine what dependency we're actually going to use
name|Dependency
name|matchingDependency
init|=
name|findMatchingDependency
argument_list|(
name|feature
operator|.
name|getFeature
...
condition|)
block|{
comment|// The feature already has a matching dependency, merge
...
expr_stmt|;
block|}
comment|// We mustn't de-duplicate here, we may have seen a feature in !add mode
name|otherFeatures
operator|.
name|put
argument_list|(
...
control|)
block|{
comment|// Match dependencies “generously” (we might be matching single-version dependencies with version ranges)
name|Dependency
...
parameter_list|)
block|{
comment|// In case of a maven build below the 'package' phase, references to the 'target/classes'
comment|// directories are passed in instead of jar-file references.
if|if
condition|(
name|file
operator|.
...
name|warn
argument_list|(
literal|"Error while reading artifact from directory"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|warn
argument_list|(
literal|"Manifest not present in the module directory "
operator|+
name|file
operator|.
name|getAbsolutePath
...
name|FeaturesCache
block|{
comment|// Maven-to-Aether Artifact cache, as parsing strings is expensive
specifier|private
specifier|final
name|SimpleLRUCache
argument_list|<
