...
begin_comment
comment|/**  *<p>The<code>Configuration</code> class encapsules the  * configuration for the event admin.</p>  *  *<p>The service knows about the following properties which are read at bundle startup:</p>  *<p>  *<code>org.apache.felix.eventadmin.ThreadPoolSize</code> - The size of the thread  *          pool.  *</p>  *  *<p>The default value is 10. Increase in case of a large amount of synchronous events  * where the<code>EventHandler</code> services in turn send new synchronous events in  * the event dispatching thread or a lot of timeouts are to be expected. A value of  * less then 2 triggers the default value. A value of 2 effectively disables thread  * pooling.</p>  *  *<p>  *<code>org.apache.felix.eventadmin.Timeout</code> - The black-listing timeout in  *          milliseconds  *</p>  *  *<p>The default value is 5000. Increase or decrease at own discretion. A value of less  * then 100 turns timeouts off. Any other value is the time in milliseconds granted  * to each<code>EventHandler</code> before it gets blacklisted.</p>  *  *<p>  *<code>org.apache.felix.eventadmin.RequireTopic</code> - Are<code>EventHandler</code>  *          required to be registered with a topic?  *</p>  *  *<p>The default is<code>true</code>. The specification says that<code>EventHandler</code>  * must register with a list of topics they are interested in. Setting this value to  *<code>false</code> will enable that handlers without a topic are receiving all events  * (i.e., they are treated the same as with a topic=*).</p>  *  *<p>  *<code>org.apache.felix.eventadmin.IgnoreTimeout</code> - Configure  *<code>EventHandler</code>s to be called without a timeout.  *</p>  *  *<p>If a timeout is configured by default all event handlers are called using the timeout.  * For performance optimization it is possible to configure event handlers where the  * timeout handling is not used - this reduces the thread usage from the thread pools  * as the timout handling requires an additional thread to call the event handler.  * However, the application should work without this configuration property. It is a  * pure optimization.  * The value is a list of string (separated by comma). If the string ends with a dot,  * all handlers in exactly this package are ignored. If the string ends with a star,  * all handlers in this package and all subpackages are ignored. If the string neither  * ends with a dot nor with a start, this is assumed to define an exact class name.</p>  *  *<p>These properties are read at startup and serve as a default configuration.  * If a configuration admin is configured, the event admin can be configured  * through the config admin.</p>  *  * @author<a href="mailto:dev@felix.apache.org">Felix Project Team</a>  */
end_comment
