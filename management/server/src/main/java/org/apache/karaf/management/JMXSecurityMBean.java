...
begin_comment
comment|/**  * Security MBean. This MBean can be used to find out whether the currently logged user can access certain MBeans  * or invoke operations on these MBeans. It can be used when building client-facing consoles to ensure that only  * operations appropriate for the current user are presented.<p/>  * This MBean does not actually invoke any operations on the given objects, it only checks permissions.  */
end_comment
...
name|COLUMNS
decl_stmt|;
comment|/**      * Checks whether the current user can invoke any methods on a JMX MBean.      *      * @param objectName the Object Name of the JMX MBean.      * @return {@code true} if there is at least one method on the MBean that the user can invoke, {@code false} else.      * @throws Exception      */
name|boolean
name|canInvoke
parameter_list|(
name|String
